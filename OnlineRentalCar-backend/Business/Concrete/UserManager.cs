...
namespace Business.Concrete
{
    ...
    public bool setPermissionsToUser(User user)
    {
      List<OperationClaim> allClaims = _userDal.SetClaims();
      if(allClaims != null)
      {
        return _userDal.AssignClaimsToUser(user, filterClaims(allClaims));
      }
      return false;
    }
    ...
    private List<OperationClaim> filterClaims(List<OperationClaim> allClaims)
    {
       ...
       List<OperationClaim> temp = allClaims
         .Where(claim => userClaims.Contains(claim.Name))
         .ToList();

      Trace.WriteLine(temp.Count);
